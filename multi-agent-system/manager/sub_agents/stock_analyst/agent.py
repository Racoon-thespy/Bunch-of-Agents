...
def get_stock_price(ticker:str) -> dict:
    """Retrieves current stock price and saves session to state."""
...
stock_analyst = Agent(
    ...
    instruction = """
    You are a helpful stock market assistant that can search up and track stock prices of the user's preference.

    When asked about stock prices : 
    1. Use get_stock_price tool to fetch the lates price of the stock(s) requested by the user
    2. Format the response to show each stock's current price and the time it was fetched
    3. If a stock price couldn't be fetched, mention it in the response
