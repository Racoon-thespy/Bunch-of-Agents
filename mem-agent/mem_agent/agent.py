...
def add_reminder(reminder: str, tool_context = ToolContext) -> dict:
    ...
    Args:
        ...
        tool_context: Context for accessing and 

    Returns: 
        A confirmation message
    """
    print(f"---- Reminder called for '{reminder}' ----")

    #Current reminders from state
    reminders = tool_context.state.get("reminders",[])

    #Add new reminders
    reminders.append(reminder)

    #Update state with new list of reminders
...
def update_reminder(index : int, updated_text : str, tool_context : ToolContext) -> dict:
    """Update and existinf reminder

    Args: 
        index : The 1 based index of theh reminder to update
        updated_text : The new text of the reminder
    ...
    print(f"---- update reminder with index : {index} with {updated_text}")
    ...
    #Update for reminders : Change for 0 indexing
    old_reminder = reminders[index-1]
    reminders[index -1] = updated_text

    #Update list with modified reminders
    tool_context.state["reminders"] = reminders
    return{
        "action" : "update_reminder",
        "index" : index,
...
def delete_reminder(index : int, tool_context : ToolContext) -> dict:
    ...
    Delete a Reminder.
    Args: 
        index : the 1-based index of reminder to delete
        tool_context : Context for accessing and updating the session state

    Returns:
        A confirmation message 
    """
    print(f"---- Delete reminder called for index : {index} ----")
    ...
    if not reminders or index < 1 or index > len(reminders):
        return{
            ...
            "status" : "error",
            "message" : f"Could not find reminder at position index : {index}. Current number of reminders : {len(reminders)}!"
        }
    #remove reminder(for 0 index riyal)
...
def update_user_name(name: str, tool_context : ToolContext) -> dict:
    ...
    Args:
        name : New user name
        tool_context : Context for accessing and updatingsession state
...
#Simplel agent for demonstration
mem_agent = Agent(
    name="mem_agent",
