...
def refund_course(tool_context = ToolContext) -> dict : 
    ...
    if course_id not in course_ids:
        return{
            "status" : "error",
            "message" : "You seem to not have purchased the course, thus no refund for you."
        }
    # Create new list of purchased courses without the refunded course
    new_purchased_courses = []
    for course in current_purchased_courses:
        #Skipping empty and non-dict etries
        if not course or not isinstance(course, dict):
            continue
        #Skipping refunded course
        if course.get("id") == course_id:
            continue
        #Add rest of the courses
        new_purchased_courses.append(course)

    # Update purchased courses in state via assignment
    tool_context.state["purchsed_courses"] = new_purchased_courses
    ...
    new_interaction_history.append(
        {"action" : "refund_course", "course_id" : course_id, "time_stamp" : current_time}
    )
    # Update interaction history in state via assignment
    tool_context.state["purchased_course"] = new_purchased_courses
...
order_agent = Agent(
    ...
    description = "Order agent for reviewing purchase history and processing refunds",
    instruction = """
