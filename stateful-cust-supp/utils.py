...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    Args:
        session_service : The session service insatnce
    ...
    try : 
        ...
        interaction_history = session.state.get("inetraction_history",[])
        
        # Add timestamp, if not alrwady present
        if "timestamp" not in entry:
            entry["timestamp"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Adding entry to interaction history
        ...
        # Create a new session with updated history
        ...
        print(f"Error updating the interaction history : {e}")
